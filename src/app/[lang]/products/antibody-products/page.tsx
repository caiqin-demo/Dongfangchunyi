        ...
        <section className="mx-auto w-[calc(100%-4rem)] max-w-panel py-16 max-[960px]:w-[calc(100%-2rem)] max-[640px]:w-[calc(100%-1.5rem)] max-[640px]:py-10">
          <div className="grid auto-rows-fr grid-cols-2 items-stretch gap-6 max-[800px]:grid-cols-1">
            {t.products.map((product) => (
              <article className="flex h-full min-w-0 flex-col rounded-product-card border border-line bg-white p-[clamp(1rem,2.5vw,2rem)] shadow-media" key={product.id}>
                ...
                <section className="mt-auto pt-7" aria-labelledby={`${product.id}-sku-title`}>
                  <h3 className="mb-3 text-lg font-bold" id={`${product.id}-sku-title`}>{t.skuLabels.title}</h3>
                  {product.skus.length > 0 ? (
                    <div className="overflow-x-auto rounded-control border border-line [contain:paint]" role="region" aria-label={`${product.title} ${t.skuLabels.title}`} tabIndex={0}>
                      <table className="w-full min-w-[620px] border-collapse text-left text-sm">
                        <thead className="bg-ui-card text-on-dark">
                          <tr>
                            <th className="px-4 py-3" scope="col">{t.skuLabels.packSize}</th>
                            <th className="px-4 py-3" scope="col">{t.skuLabels.catalogNumber}</th>
                            <th className="px-4 py-3" scope="col">{t.skuLabels.shippingOrigin}</th>
                            <th className="px-4 py-3" scope="col">{t.skuLabels.availability}</th>
                          </tr>
                        </thead>
                        <tbody>
                          {product.skus.map((sku) => (
                            <tr className="border-t border-line" key={sku.catalogNumber}>
                              ...
                              <td className="px-4 py-3 text-ink-muted">{t.skuLabels.availabilityNote}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    </div>
                  ) : <p className="rounded-control border border-line bg-ui-subtle p-4 leading-[1.7] text-ink-muted">{t.skuLabels.incompleteSource}</p>}
                </section>
                <div className="mt-6 flex min-h-60 items-end justify-between gap-4">
                  ...
                  <Image
                    ...
                    src={experimentImages[product.id].src}
                    width={experimentImages[product.id].width}
                    height={experimentImages[product.id].height}
