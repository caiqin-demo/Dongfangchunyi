    ...
    <>
      <div className="grid grid-cols-2 items-start gap-12 max-page:grid-cols-1">
        ...
        <section aria-labelledby={sampleStandardsTitleId}>
          <h3 className="m-0 text-service-section-title text-white" id={sampleStandardsTitleId}>{system.sampleStandardsTitle}</h3>
          <ul className="mt-5 grid list-none grid-cols-2 gap-4 p-0 max-compact:grid-cols-1">
            {system.sampleStandards.map((standard) => (
              <li className="rounded-control border border-line-dark bg-ui-card p-5" key={standard.id}>
                <p className="m-0 text-service-card-title text-white">{standard.title}</p>
                <p className="mt-2 mb-0 text-service-card-body text-on-dark-muted">{standard.description}</p>
              </li>
            ))}
          </ul>
        </section>
      ...
      <section className="mt-12 rounded-product-card border border-line-dark bg-ui-card p-6" aria-labelledby={catalogTitleId}>
        <h3 className="m-0 text-service-current-title text-white" id={catalogTitleId} lang="en">{system.cdnaLibrary.title}</h3>
        <div className="mt-6 grid grid-cols-4 gap-6 max-page:grid-cols-2 max-compact:grid-cols-1">
          {system.cdnaLibrary.categories.map((category) => (
            <section className="min-w-0 border-l border-line-dark pl-6 first:border-l-0 first:pl-0 max-page:odd:border-l-0 max-page:odd:pl-0 max-compact:border-l-0 max-compact:pl-0" aria-labelledby={`yeast-two-hybrid-${system.id}-${category.id}-title`} key={category.id}>
              <h4 className="m-0 text-service-card-title text-white" id={`yeast-two-hybrid-${system.id}-${category.id}-title`} lang="en">{category.title}</h4>
              <ul className="mt-4 list-none space-y-2 p-0 text-service-card-body text-on-dark-muted">
