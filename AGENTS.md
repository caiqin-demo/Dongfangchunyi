...
- The Landing page Hero is an approved independent implementation. It must not inherit the Hero rendering approach used by other pages.
...
- Apply the shared UI acceptance baseline after every relevant Hero layout, typography, copy, or localization change. A Hero change passes only when all copy remains visible without clipping or horizontal overflow, every CTA meets its required computed dimensions, and all Hero content remains inside the section.
...
- Changes to colors, fonts, cards, or the Footer must apply the shared UI acceptance baseline and additionally inspect final composited contrast, focus and interaction semantics, and relevant network requests.
...
- At a changed breakpoint, related components must enter compatible layout states without overlap or unintended horizontal page overflow.
- Give the Hero heading and the body content it introduces a deliberate shared outer alignment line. A section may use a narrower readable inner width while preserving that outer alignment.
- Define the approved page measure and responsive gutters once as the single source of truth for page-level content geometry. Headers, Footers, non-Landing Hero content, page sections, panels, and shared templates must derive their outer alignment from that source and must not independently recreate or recalculate equivalent `max-width`, margin, padding, or gutter rules. Any intentional difference requires an explicitly named and approved layout variant.
- Treat full-bleed site-shell paint and content geometry as separate layers. A site-wide Header or Footer may extend its background, border, or shadow across the viewport, but its brand identity, navigation, language selector, utility actions, legal text, and other content must sit inside a centered inner container governed by the approved page-measure and gutter tokens.
- Do not apply edge-to-edge distribution such as `justify-between` directly to an unconstrained full-width Header or Footer when it allows content groups to drift toward the viewport edges on wide screens. Apply alignment and distribution inside the bounded inner container; any deliberately edge-aligned shell must be an explicit named layout variant with an approved design reason.
- Anchor compact-screen navigation, language controls, menus, Footer groups, and other positioned shell elements to the same responsive inner container rather than independently positioning them against the viewport. Changing the shell layout must preserve the shared content alignment line across wide, desktop, breakpoint-adjacent, and compact layouts.
...
- Apply the shared UI acceptance baseline while verifying tables after both horizontal and vertical scrolling. Confirm sticky behavior, header association, reading order, focus visibility, and the absence of overlap.
...
## Shared non-Landing page Hero image delivery requirements

- These requirements apply to every page Hero except the independently implemented Landing page Hero.
- Use one shared default Hero pipeline for every non-Landing page. Keep the authorized source image unchanged in the repository, import the page asset statically, and render it through the shared page template as a decorative `next/image` with `fill`, `sizes="100vw"`, the shared `object-cover object-center` presentation, and the shared full-section overlay. Do not choose a different delivery or responsive strategy merely because a page or source file is different.
- Let Next.js generate responsive optimized output for the default pipeline. Do not manually convert an ordinary supplied Hero image, disable optimization, or bypass responsive image selection unless the default has produced a verified visual-fidelity, transfer-size, caching, or browser-compatibility problem.
- Keep visual treatment, responsive fitting, resource optimization, and loading priority as separate concerns. A visual variant must not implicitly toggle `object-fit`, `unoptimized`, eager loading, preload, or fetch priority; each non-default behavior requires its own explicit typed option and evidence.
- Treat the shared template's default crop, focal position, overlay, foreground layering, and text contrast as the accepted non-Landing page design. Discuss an exception only after the supplied image has been inspected in the actual page and the default produces an identifiable visual problem at a required locale, viewport, or zoom level. Record the problem and obtain explicit user approval before adding the smallest named exception.
- For an approved fidelity-sensitive or composited exception, preserve the authorized original and create a distinctly named derivative. A lossless WebP may be statically imported and served with `unoptimized` only when pixel or visual comparison proves fidelity, measured bytes justify the derivative, and the emitted hashed asset receives the intended immutable cache policy. Do not use the exception as a new default for ordinary Hero photographs.
- At responsive reflow and browser zoom, the image and overlay layers must remain visually continuous across the complete Hero. `object-contain`, bottom-only imagery, or a partial-height overlay must not reveal the section background, an image boundary, a hard gradient seam, or an uncovered contrast region. If preserving a focal object requires a compact composition, implement and approve that composition as a named visual exception rather than combining unrelated mobile utility overrides.
- Apply eager loading, preload, or high fetch priority according to the Performance verification requirements, based on evidence for the route and tested viewport. A Hero's visual variant or file format alone does not prove that it is the LCP resource.
- A non-Landing page Hero change passes only when the shared UI acceptance baseline and the Page image asset requirements pass for every supported locale, and browser checks additionally confirm the default and any approved exception at 400% reflow and Chrome's 500% zoom. Inspect the complete Hero layering, computed `object-fit` and `object-position`, crop and focal visibility, composited text contrast, initial-HTML request discovery, responsive candidate selection, decoded format, transferred bytes, cache policy, loading priority, and measured LCP behavior in a production build.
...
- As part of the shared UI acceptance baseline's required human review, manually verify landmarks, heading order, keyboard traversal, focus visibility, skip-link behavior, document and link languages, target dimensions, meaningful alternative text, screen-reader announcements for changed interactions, and layout at 200% and 400% zoom. Automated checks such as axe are supporting evidence only.
...
- A multilingual routing change passes only when the production build identifies the localized content routes as static where expected, when the final URL, document language, localized metadata, canonical and alternate links, active language state, same-origin language switching, and content parity are correct for every supported locale, and when the shared UI acceptance baseline passes at the required locale and viewport checks.
...
- Adding or restructuring localized content passes only when `npm run typecheck` confirms the shared contract, when every supported locale renders the expected item counts, labels, destinations, active-language state, and content order without missing content or incorrect asset or behavior associations, and when the shared UI acceptance baseline passes at the required locale and viewport checks.
