...
- Use a subagent only when the current user message explicitly requests one. Do not carry a prior message's agent choice, delegation preference, or model instruction into a later request; otherwise the main agent performs the work directly.
